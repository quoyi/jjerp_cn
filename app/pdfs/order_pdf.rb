class OrderPdf < Prawn::Document
  ...
  def initialize(length, width, ids, order_id)
    ...
    @ids.downto(1).each_with_index do |key, i|
      ...
      pad_top(1) {text "终端: "}
      pad_top(1) {text_box "#{@order.indent.customer[0, 32]}", at: [30, cursor + 13]}
      text " "
      pad_top(1) {text "产品: #{@order.order_category.try(:name)[0, 16]}"}
      pad_top(1) {text "备注: "}
      # 字体高度12 ，上下內补1 =>  12 + 1 + 1 = 14
      pad_top(1) {text_box "#{@order.note[0,32]}", at: [30, cursor + 13]}
      text " "
      ...
      text "共 #{@ids} 件 第 #{@ids} - #{i+1} 件", size: 9, align: :right, valign: :bottom
    end
    # autoprint  # 自动打印
    print  # 手动调用 打印 方法
  end
