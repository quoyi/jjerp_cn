class SentListPdf < Prawn::Document
  ...
  def print_content
    ...
    tbody = [["序号", "地区", "收货人", "联系方式", "订单编号", "橱", "衣", "门", "配", "合计", "代收", "物流名称", "备注"]]
    ...
    text "发货时间：["+ @sent_list.created_by.to_s + "][数量合计：" + @sent_list.sents.map{|sent| sent.cupboard + 
         sent.robe + sent.door + sent.part}.flatten.sum.to_s + "]件", align: :right
  end
end