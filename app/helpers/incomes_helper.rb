module IncomesHelper
  ...
  def create_incomes(timestamp, incomes)
    directory = Rails.root.join('public/excels/incomes')
    ...
    wb = WriteExcel.new(Rails.root.join("public/excels/incomes/#{timestamp}.xls"))
    ws = wb.add_worksheet
    ...
    incomes.each_with_index do |income, index|
      ws.write_row("A#{row_num}", [index + 1, income.reason, income.bank.bank_card, income.income_at.strftime('%Y-%m-%d'),
